...
export default async function DevotionalPage({
  ...
  // Filter only Devotional products
  const devotionalProducts = products.filter(
    (product: any) =>
      product.category === "Devotional" ||
      product.categories?.includes("Devotional")
  );
  ...
  const paginatedProducts = devotionalProducts.slice(
    startIndex,
    endIndex
  );

  const totalPages = Math.ceil(
    devotionalProducts.length / PRODUCTS_PER_PAGE
  );
